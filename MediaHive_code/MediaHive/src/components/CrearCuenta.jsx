...
function CrearCuenta() {
  ...
  const onSubmit = handleSubmit(data => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      /*
      const credenciales = [`${data.correoElectronico}, ${data.contrasena}`];
      const credencialesJSON = JSON.stringify(credenciales);
      if (localStorage.getItem('credencialesSaved') === null) {
        localStorage.setItem('credencialesSaved', '[' + credencialesJSON + ']');
      } else {
        let credencialesSaved = JSON.parse(localStorage.getItem('credencialesSaved'));
        if (!credencialesSaved.some(savedCred => savedCred[0] === credenciales[0] && savedCred[1] === credenciales[1])) {
          credencialesSaved.push(credencialesJSON);
          localStorage.setItem('credencialesSaved', JSON.stringify(credencialesSaved));
        }
      }
      navigate(`/Inicio/${data.nombreUsuario}`);
      */
